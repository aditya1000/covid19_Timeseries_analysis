...
SIS_model_simulate <- function(State_nam, 
  ...
  ##for(State_nam in unique(df1$Name.of.State...UT)){
    ...
    if(nrow(df) >= 3){
      ...
      df$Total.Confirmed.cases <- as.numeric(df$Total.Confirmed.cases)
      df$Cured.Discharged.Migrated <- as.numeric(df$Cured.Discharged.Migrated)
      ...
      #fit$Predicted_recovered <- as.numeric(fit$Predicted_recovered)
      fit$Predicted_Infected <- as.numeric(fit$Predicted_Infected)
...
SIS_model_simulate(args[1],as.numeric(args[2]),as.numeric(args[3]),as.numeric(args[4]),as.numeric(args[5]))

# my_out <- SIS_model_simulate("Delhi", 100, 10, 2,25)
# my_out <- data.frame(my_out[2])
